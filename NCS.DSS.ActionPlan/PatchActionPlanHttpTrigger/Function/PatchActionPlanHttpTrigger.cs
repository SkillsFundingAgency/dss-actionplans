...
namespace NCS.DSS.ActionPlan.PatchActionPlanHttpTrigger.Function
{
    public class PatchActionPlanHttpTrigger
    {
        ...
        public async Task<IActionResult> Run([HttpTrigger(AuthorizationLevel.Anonymous, "patch", Route = "Customers/{customerId}/Interactions/{interactionId}/ActionPlans/{actionPlanId}")] HttpRequest req, string customerId, string interactionId, string actionPlanId)
        {
            ...
            {
                _logger.LogInformation("{ActionPlanPatchRequest} object is NULL. Correlation GUID: {CorrelationGuid}", nameof(actionPlanPatchRequest), correlationGuid);
                return new UnprocessableEntityObjectResult(req);
            }

            _logger.LogTrace("Retrieved resource from request body. Correlation GUID: {CorrelationGuid}", correlationGuid);
                        
            _logger.LogTrace("Attempting to set IDs for Action Plan PATCH. Correlation GUID: {CorrelationGuid}", correlationGuid);
            actionPlanPatchRequest.SetIds(touchpointId, subcontractorId);
            _logger.LogTrace("IDs successfully set for Action Plan PATCH. Correlation GUID: {CorrelationGuid}", correlationGuid);
            ...
            _logger.LogTrace("Attempting to PATCH Action Plan resource.");
            ...
            _logger.LogTrace("Attempting to deserialize the PATCH Action Plan resource.");
            try
            {
            ...
            {   
                _logger.LogInformation("Action Plan validation object is NULL. Correlation GUID: {CorrelationGuid}", correlationGuid);
                return new UnprocessableEntityObjectResult(req);
            }

            _logger.LogTrace("Attempting to get Date and Time of Session. SessionID: {SessionID}", actionPlanValidationObject.SessionId);            
            ...
            _logger.LogTrace("Attempting to validate {ActionPlanValidationObject} object", nameof(actionPlanValidationObject));
            ...
            _logger.LogTrace("Successfully validated {ActionPlanValidationObject}", nameof(actionPlanValidationObject));
                        
            _logger.LogTrace("Attempting to PATCH Action Plan in Cosmos DB. Action Plan GUID: {ActionPlanGuid}", actionPlanGuid);
            ...
            {
                _logger.LogTrace("Successfully PATCH an Action Plan in Cosmos DB. Action Plan GUID: {ActionPlanGuid}", actionPlanGuid);
                
                _logger.LogTrace("Attempting to send message to Service Bus Namespace. Action Plan GUID: {ActionPlanGuid}", actionPlanGuid);
                await _actionPlanPatchService.SendToServiceBusQueueAsync(updatedActionPlan, customerGuid, apimUrl);
                _logger.LogTrace("Successfully sent message to Service Bus. Action Plan GUID: {ActionPlanGuid}", actionPlanGuid);
            }

            if (updatedActionPlan == null)
            {
                _logger.LogInformation("PATCH request unsuccessful. Action Plan GUID: {ActionPlanGuid}", actionPlanGuid);
                return new BadRequestObjectResult(actionPlanGuid);
            }

            _logger.LogTrace("Function {FunctionName} has finished invoking", nameof(PatchActionPlanHttpTrigger));
