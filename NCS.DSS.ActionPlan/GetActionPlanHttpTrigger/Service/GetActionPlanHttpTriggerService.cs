...
namespace NCS.DSS.ActionPlan.GetActionPlanHttpTrigger.Service
{
    public class GetActionPlanHttpTriggerService : IGetActionPlanHttpTriggerService
    {
        ...
        {
            _logger.LogTrace("Attempting to get Action Plans for Customer. Customer ID: {CustomerId}.", customerId);
            ...
            _logger.LogTrace("{Count} Action Plan(s) successfully retrieved. Customer GUID: {CustomerId}", actionPlans.Count, customerId);
            return actionPlans;
        }
    }
}